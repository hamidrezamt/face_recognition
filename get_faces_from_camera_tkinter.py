# Face register GUI with tkinter
...
class Face_Register:
    def __init__(self):
        ...
        # modify window size here if needed
        ...
        # CAP_AVFOUNDATION for Macos
        self.cap = cv2.VideoCapture(0,cv2.CAP_AVFOUNDATION)  # Get video stream from camera
    ...
    def GUI_clear_data(self):
        # Delete the previously saved face data folder, delete "/data_faces_from_camera/person_x/"...
    ...
    # If there is a previously entered face, the sequence number of person_x before is entered according to person_x+1
    def check_existing_faces_cnt(self):
        if os.listdir("data/data_faces_from_camera/"):
            # Get the last recorded face number
            
            person_list = [f for f in os.listdir("data/data_faces_from_camera/") if not f.startswith('.')]
            person_num_list = []
        ...
        # If it is stored for the first time or there is no previously recorded face, start recording according to person_1
        else:
            print("there wasn't")
            self.existing_faces_cnt = 0
    ...
    def create_face_folder(self):
        ...
        logging.info("\n%-40s %s", "New face folder / Create folders:", self.current_face_dir)

        self.ss_cnt = 0  # Clear the face counter / Clear the cnt of screen shots
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    print("self.face_ROI_height * 2: " +str(self.face_ROI_height * 2))
                    print("self.face_ROI_width * 2: " + str(self.face_ROI_width * 2))
    ...
    # getting face / Main process of face detection and saving
    def process(self):
        ...
        if ret:
            ...
            # Face detected
            if len(faces) != 0:
                # Draw rectangle and Show the ROI of faces
                for k, d in enumerate(faces):
                    ...
                    # Determine whether the rectangular frame of the face exceeds 480x640, If the size of ROI > 480x640
                    if (d.right() + self.ww) > 1280 or (d.bottom() + self.hh > 960) or (d.left() - self.ww < 0) or (d.top() - self.hh < 0):
                        print("d.right()  + d.left() + self.face_ROI_width  + self.ww " + str(d.right())  + " " + str(d.left()) + " " + str(self.face_ROI_width)  + " " + str(self.ww))
                        print("d.bottom() + d.top()  + self.face_ROI_height + self.hh " + str(d.bottom()) + " " + str(d.top())  + " " + str(self.face_ROI_height) + " " + str(self.hh))
