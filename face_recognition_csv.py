...
class Face_Recognizer:
    ...
    def feature_extraction_csv(self):
        ...
        with open("data/features_all.csv", "w", newline="") as csvfile:
            ...
            for person in person_list:
                ...
                logging.info("%sperson_%s", self.path_photos_from_camera, person)
                features_mean_personX = self.return_features_mean_personX(self.path_photos_from_camera + person)
    ...
    def head_direction(self, image):
        try:
            ...
            logging.debug("actualy there is no Face in image temp")
            dir = 0
            ...
            if self.get_face_database():
                    ...
                    # 6.2.2 If the number of faces increased
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < self.similarity_thresh:
                                ...
                                cv2.imwrite("debug/debug_" + str(self.frame_cnt) + ".png", img_tmp) # Dump current frame image if needed

                                logging.debug("blur_detection(): " + str(self.blur_detection(img_tmp)))
                                logging.debug("close_eye_detection(): " + str(self.close_eye_detection(shape)))
                                logging.debug("head_direction(): " + str(self.head_direction(img_tmp)))
                                if self.blur_detection(img_blank) > self.blur_thres and self.close_eye_detection(shape) > self.eye_ar_thresh and self.head_direction(img_tmp) :
                                    ...
                                    cv2.imwrite(current_face_dir + "/img_face.jpg", img_blank)
                                    logging.info("%-40s %s/img_face.jpg", "Save into：", str(current_face_dir))
                ...
                if len(faces) != 0:
                    ...
                    for k, d in enumerate(faces):
                        ...
                        cv2.imwrite("debug/debug_" + str(self.frame_cnt) + "no_database.png", img_tmp) # Dump current frame image if needed
